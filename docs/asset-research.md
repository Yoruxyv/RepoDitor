...
each file on every `repoditor-icon:` request, and React falls back to Phosphor on any failure. No
game image is copied into RepoDitor, its derived cache, tests, or installer. Icon availability never
