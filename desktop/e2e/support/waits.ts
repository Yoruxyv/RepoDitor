...
export async function waitForSafeSave(
  page: Page,
  surface: SaveSurface,
): Promise<void> {
...
export async function waitForStaleSave(
  page: Page,
  surface: SaveSurface,
): Promise<void> {
...
export async function waitForGameRunningDialog(page: Page): Promise<void> {
  await expect(
    page.getByRole("heading", { name: "R.E.P.O. is currently running" }),
  ).toBeVisible({
    timeout: CROSS_PROCESS_TIMEOUT_MS,
  });
...
export async function waitForGameDialogClosed(page: Page): Promise<void> {
  ...
  await expect.poll(
    () => page.getByTestId("editor-content").evaluate((editor) => (
