...
export function DiscoveryState({ saveRoot, status }: DiscoveryStateProps) {
  const content =
    status === "missing"
      ? {
          icon: <Folder aria-hidden="true" size={28} weight="regular" />,
      ...
      : status === "unreadable"
        ? {
            icon: <WarningCircle aria-hidden="true" size={28} weight="regular" />,
        ...
        : {
            icon: <FolderOpen aria-hidden="true" size={28} weight="regular" />,
  ...
  return (
    <section className="rounded-sm border border-line bg-surface p-6 sm:p-8">
      ...
      <div className="mt-6 border-t border-line pt-4">
        ...
        <PathText
          className="mt-1 font-mono text-xs leading-5 text-secondary"
          path={saveRoot}
        />
      </div>
    </section>
...
export function DiscoveryFailure({ error, onRetry }: DiscoveryFailureProps) {
  return (
    <section className="mx-auto max-w-2xl border-l-2 border-danger bg-surface px-6 py-8 sm:px-8">
      ...
      <p className="mt-4 max-w-[58ch] text-sm leading-6 text-secondary">
