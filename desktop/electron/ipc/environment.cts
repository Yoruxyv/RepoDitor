...
function parseSaveSummary(
  ...
  const modifiedAt = readString(
    value.lastModified,
    "save lastModified",
  );
  if (Number.isNaN(Date.parse(modifiedAt))) {
    throw new EnvironmentProtocolError(
      "Invalid save lastModified.",
    );
  }
...
function parseSaveRootStatus(
  value: unknown,
): SaveRootStatus {
  if (
    typeof value !== "string" ||
    !isSaveRootStatus(value)
  ) {
    throw new EnvironmentProtocolError(
      "Invalid saveRootStatus.",
    );
  }
  return value;
}

function isSaveRootStatus(
  value: string,
): value is SaveRootStatus {
  return (
    value === "available" ||
    value === "missing" ||
    value === "unreadable"
...
function isGameDiscoveryStatus(
  value: string,
): value is GameDiscoveryStatus {
  return (
    value === "found" ||
...
function parseEnvironment(
  ...
  if (!Array.isArray(value.saves)) {
    throw new EnvironmentProtocolError(
      "Invalid saves collection.",
    );
  }
  const saves = value.saves.map(
    parseSaveSummary,
  );

  if (
    typeof value.saveCount !== "number" ||
    value.saveCount !== saves.length
  ) {
    throw new EnvironmentProtocolError(
      "Invalid saveCount.",
    );
  ...
  const gameStatus = parseGameStatus(
    value.gameStatus,
  );
  const gameDetected = readBoolean(
    value.gameDetected,
    "gameDetected",
  );
  if (
...
function getPublicError(
  ...
  if (error instanceof PythonClientError) {
    const messages = {
      python_unavailable:
        "The Python discovery service is unavailable.",
      process_failed:
        "The Python discovery service failed.",
      process_timeout:
        "The Python discovery service timed out.",
      empty_response:
        "The Python discovery service returned no data.",
      malformed_response:
        "The Python discovery service returned malformed data.",
    } as const;
    return {
      code: error.code,
  ...
  if (error instanceof EnvironmentProtocolError) {
    return {
      ...
      message:
        "The Python discovery response did not match the desktop contract.",
    };
  }
  ...
  return {
    code: "internal_error",
    message:
      "The desktop discovery bridge failed unexpectedly.",
  };
}
